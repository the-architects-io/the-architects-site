...
export default function LootBoxDetailPage({ params }: { params: any }) {
  ...
  const handleTransferCostTokens = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`${BASE_URL}/api/claim-loot-box`, {
      ...
      showToast({
        ...
        secondaryMessage: `You received ${data?.payout?.amount}x ${data?.payout?.token?.name}.`,
      });
  ...
  const fetchUserHeldCostTokensViaMintAddress = useCallback(async () => {
    ...
    const { data } = await axios.post(
      ...
      {
        mintAddresses: [costItem.token.mintAddress],
  ...
  }, [wallet.publicKey, connection, costItem, hasFetchedUserHeldCostTokens]);
